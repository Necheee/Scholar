...
**Root Cause:**
PowerShell execution policies block running `npm.ps1` natively without additional setup.

**Resolution:**
Always use `npm.cmd` when executing Node scripts in this environment (e.g. `npm.cmd run lint`, `npm.cmd run build`). This bypasses the PowerShell script restriction and correctly invokes the Node package manager executable.
...
**Resolution:**
With `npm.cmd` unblocked, the build commands (`npm.cmd run lint` and `npm.cmd run build`) have been run against the codebase (now up to Phase 5). All linters pass with 0 warnings/errors, and Vite completes the production build successfully in ~720ms. The application builds flawlessly.
